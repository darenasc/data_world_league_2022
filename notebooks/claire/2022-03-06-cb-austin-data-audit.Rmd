...
grouped_waste_no_outlier %>%
  filter(lubridate::year(day) > 2004,
         day <= '2021-06-01') %>%
  ggplot(aes(day, av_weight)) +
  geom_line() +
  theme(axis.text.x = element_text(angle = 45)) +
  facet_wrap(Load.Type ~.)+
  labs(title = 'Time series of different types of waste',
       ...
       x = NULL,
       y = 'Average daily waste in pounds')
```
...
clean_df <- selected_waste_w_outliers %>% 
  ...
  left_join(gdp %>% select(year, gdp), by = 'year') %>%
  group_by(date, year, month, wday, Load.Type, Route.Type, Dropoff.Site, Route.Number, outlier, total_pop, annualised_growth, gdp) %>%
...
clean_df %>%
  group_by(year) %>%
  summarise(waste_pp = sum(daily_weight)/first(total_pop)) %>% ggplot(aes(year, waste_pp)) + geom_line()
```
